(() => {
  ...
  function renderTrace() {
    ...
    const wrap = document.querySelector(".trace-canvas-wrap"); const canvas = document.getElementById("trace-canvas"); const width = wrap.clientWidth; const height = Math.max(document.querySelector(".trace-layout").clientHeight, rows.length * TRACE_ROW_HEIGHT); const ctx = canvasSize(canvas, width, height); const domain = activeDomain();
    ...
    let index = 0;
    for (const row of rows) {
      const y = index++ * TRACE_ROW_HEIGHT; if (row.role) { ctx.fillStyle="#edf3fc"; ctx.fillRect(0,y,width,TRACE_ROW_HEIGHT); continue; }
      const session = row.session; const sessionSpan = sessionSpans.get(session.session_id); ctx.fillStyle = session.session_id === state.sessionId ? "#dceaff" : "#f8fafc"; ctx.fillRect(0,y,width,TRACE_ROW_HEIGHT);
  ...
  function toolOperationCalls(tool) { const attrs=tool.attributes||{}, outer=attrs.name||"tool", nested=Array.isArray(attrs.nested_calls)?attrs.nested_calls:[]; if (!nested.length) return [{command:outer,commandArguments:""}]; return nested.map(call=>{ const commandPart=[call&&call.command_name,call&&call.command_operation].filter(Boolean).join(" ") || call&&call.command_label; const command=[outer,call&&call.tool,commandPart].filter(Boolean).join(" → "); const label=call&&call.command_label || ""; const prefix=[call&&call.command_name,call&&call.command_operation].filter(Boolean).join(" "); const commandArguments=call&&call.command_arguments || (prefix && label.startsWith(prefix) ? label.slice(prefix.length).trim() : ""); return {command,commandArguments}; }); }
  function operation(event, tool) { if (tool) return toolOperationCalls(tool).map(call=>[call.command,call.commandArguments].filter(Boolean).join(" ")).join(" · "); const d=eventDetails(event); if (event.kind === "sub_agent_activity") return [d.agent_path,d.kind].filter(Boolean).join(" · ") || event.kind; if ((event.kind||"").includes("message")) return [d.author,d.recipient].filter(Boolean).join(" → ") || event.kind; return event.kind || event.payload_type || event.outer_type; }
  function operationHtml(row) { if (!row.tool) return esc(row.op); return toolOperationCalls(row.tool).map(call=>`<div class="operation-call"><span class="operation-command">${esc(call.command)}</span>${call.commandArguments ? `<span class="operation-arguments">${esc(call.commandArguments)}</span>` : ""}</div>`).join(""); }
