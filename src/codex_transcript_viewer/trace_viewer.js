(() => {
  ...
  function toolOperationCalls(tool) { const attrs=tool.attributes||{}, outer=attrs.name||"tool", nested=Array.isArray(attrs.nested_calls)?attrs.nested_calls:[]; if (!nested.length) return [{command:outer,arguments:""}]; return nested.map(call=>{ const commandPart=[call&&call.command_name,call&&call.command_operation].filter(Boolean).join(" ") || call&&call.command_label; const command=[outer,call&&call.tool,commandPart].filter(Boolean).join(" → "); const label=call&&call.command_label || ""; const prefix=[call&&call.command_name,call&&call.command_operation].filter(Boolean).join(" "); const arguments=call&&call.command_arguments || (prefix && label.startsWith(prefix) ? label.slice(prefix.length).trim() : ""); return {command,arguments}; }); }
  function operation(event, tool) { if (tool) return toolOperationCalls(tool).map(call=>[call.command,call.arguments].filter(Boolean).join(" ")).join(" · "); const d=eventDetails(event); if (event.kind === "sub_agent_activity") return [d.agent_path,d.kind].filter(Boolean).join(" · ") || event.kind; if ((event.kind||"").includes("message")) return [d.author,d.recipient].filter(Boolean).join(" → ") || event.kind; return event.kind || event.payload_type || event.outer_type; }
  function operationHtml(row) { if (!row.tool) return esc(row.op); return toolOperationCalls(row.tool).map(call=>`<div class="operation-call"><span class="operation-command">${esc(call.command)}</span>${call.arguments ? `<span class="operation-arguments">${esc(call.arguments)}</span>` : ""}</div>`).join(""); }
