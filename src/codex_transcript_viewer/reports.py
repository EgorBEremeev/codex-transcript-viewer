"""CSV report builders over immutable breakdown, spans, and metrics documents."""
...
_CONTENT_KEYS = ("arguments_size", "content_size", "input_size", "message_size", "output_size", "summary_size")
...
def build_sessions_table_csv(document: dict[str, Any]) -> str:
    """Return one session row plus a final tree aggregate row."""
    fields = [
        "scope", "session_id", "parent_session_id", "agent_path", "thread_source",
        "native_by_kind", *[f"context_{key}" for key in _CONTENT_KEYS],
        *[f"token_{key}" for key in _TOKEN_KEYS], "rate_limit_primary_used_percent",
        "total_tool_time_ms", "total_reasoning_time_ms",
    ]

    def row(scope: str, session: dict[str, Any], metrics: dict[str, Any]) -> dict[str, Any]:
        context_material = metrics.get("context_material", {})
        context = context_material.get("by_content_kind", context_material)
        reported = metrics.get("reported_token_usage", {})
        usage = reported.get("last_cumulative") or reported.get("sum_session_cumulative", {})
        rate = metrics.get("rate_limits", {}).get("primary", {}).get("used_percent")
        return {
            "scope": scope,
            ...
            "native_by_kind": json.dumps(metrics.get("events", {}).get("native_by_kind", {}), ensure_ascii=False, sort_keys=True),
            **{f"context_{key}": _number(context.get(key)) for key in _CONTENT_KEYS},
            **{f"token_{key}": _number(usage.get(key)) for key in _TOKEN_KEYS},
            "rate_limit_primary_used_percent": _number(rate),
            "total_tool_time_ms": _number(metrics.get("total_tool_time_ms")),
            "total_reasoning_time_ms": _number(metrics.get("total_reasoning_time_ms")),
        }

    rows = [row("session", session, session.get("metrics", {})) for session in document.get("sessions", []) if isinstance(session, dict)]
    tree = document.get("tree_metrics") if isinstance(document.get("tree_metrics"), dict) else {}
    rows.append(row("tree", {}, tree))
    return _csv(rows, fields)


def build_events_table_csv(breakdown: dict[str, Any], spans: dict[str, Any], until_ms: int | None = None) -> str:
    """Export the same logical event rows as the trace table for every session."""
    fields = [
        "session_id", "agent_path", "timestamp", "duration_ms", "span_kind", "event_kind", "operation",
    ...
    for session in breakdown.get("sessions", []):
        if not isinstance(session, dict):
            continue
        events = [event for event in session.get("events", []) if isinstance(event, dict) and (until_ms is None or not isinstance(event.get("timestamp_ms"), int) or event["timestamp_ms"] <= until_ms)]
        cumulative = 0
        ...
        for event in events:
            ...
            rows.append({
                "session_id": session.get("session_id", ""), "agent_path": session.get("agent_path", ""), "timestamp": event.get("timestamp", ""),
                "duration_ms": _number(duration), "span_kind": span_kind, "event_kind": event.get("kind", ""), "operation": operation,
